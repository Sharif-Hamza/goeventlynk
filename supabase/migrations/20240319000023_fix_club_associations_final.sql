...
BEGIN
    EXECUTE (
        ...
        FROM pg_policies 
        WHERE tablename IN ('events', 'announcements', 'event_registrations', 'announcement_reactions', 'profiles', 'storage.objects')
    );
END $$;

-- Ensure clubs table exists and has correct data
