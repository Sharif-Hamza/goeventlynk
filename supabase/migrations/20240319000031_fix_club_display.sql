...
-- Create function to handle club name updates
CREATE OR REPLACE FUNCTION update_club_name()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.club_id IS NOT NULL THEN
    NEW.club_name := (SELECT name FROM clubs WHERE id = NEW.club_id);
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Add club_name column to events and announcements
ALTER TABLE events
...
-- Create triggers to update club_name
DROP TRIGGER IF EXISTS update_event_club_name ON events;
CREATE TRIGGER update_event_club_name
  BEFORE INSERT OR UPDATE ON events
  FOR EACH ROW
  EXECUTE FUNCTION update_club_name();

DROP TRIGGER IF EXISTS update_announcement_club_name ON announcements;
CREATE TRIGGER update_announcement_club_name
